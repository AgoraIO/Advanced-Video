...
BOOL CAgoraObject::EnableScreenCapture(HWND hWnd, int nCapFPS, const LPRECT lpRect, BOOL bEnable, int nBitrate)
{
	ASSERT(m_lpAgoraEngine != NULL);

	int		ret = 0;
	agora::rtc::Rectangle	rect;
	agora::rtc::Rectangle   rcClient;
	RtcEngineParameters rep(*m_lpAgoraEngine);
	...
	if (bEnable) {
		agora::rtc::Rectangle   screenRegion;
		if (hWnd == NULL){
			RECT rcDesktop;
			::GetWindowRect(GetDesktopWindow(), &rcDesktop);
			screenRegion.x = 0;
			screenRegion.y = 0;
			capParam.dimensions.width = screenRegion.width = rcDesktop.right - rcDesktop.left;
			capParam.dimensions.height = screenRegion.height = rcDesktop.bottom - rcDesktop.top;
		}

		if (lpRect != NULL) {
			rect.x = lpRect->left;
			rect.y = lpRect->top;
			capParam.dimensions.width = rect.width = lpRect->right - lpRect->left;
			capParam.dimensions.height = rect.height = lpRect->bottom - lpRect->top;

			if (hWnd != NULL){
				ret = m_lpAgoraEngine->startScreenCaptureByWindowId(hWnd, rect, capParam);
			}
			else{
				ret = m_lpAgoraEngine->startScreenCaptureByScreenRect(screenRegion, rect, capParam);
			}
		}
		else{
			if (hWnd != NULL) {
				...
				rect.y = rcClient.top;
				capParam.dimensions.width = rect.width = rcClient.right - rcClient.left;
				capParam.dimensions.height = rect.height = rcClient.bottom - rcClient.top;
				ret = m_lpAgoraEngine->startScreenCaptureByWindowId(hWnd, rect, capParam);
			}
			else{
				ret = m_lpAgoraEngine->startScreenCaptureByScreenRect(screenRegion, rect, capParam);
...
BOOL CAgoraObject::PushVideoFrame(LPCVOID lpYUVBuffer, int nWidth, int nHeight)
{
	agora::util::AutoPtr<agora::media::IMediaEngine> mediaEngine;
	mediaEngine.queryInterface(m_lpAgoraEngine, agora::AGORA_IID_MEDIA_ENGINE);
	...
	LPBYTE lpYUV = reinterpret_cast<LPBYTE>(const_cast<LPVOID>(lpYUVBuffer));
