...
void CExtCaptureDlg::DoDataExchange(CDataExchange* pDX)
{
	...
	DDX_Control(pDX, IDC_CHECK_PUSHVIDEO, m_ckExtPushVideo);
}
...
BEGIN_MESSAGE_MAP(CExtCaptureDlg, CDialogEx)
	...
	ON_WM_PAINT()
	ON_WM_SHOWWINDOW()
	ON_BN_CLICKED(IDC_CHECK_PUSHVIDEO, &CExtCaptureDlg::OnBnClickedCheckPushvideo)
END_MESSAGE_MAP()
...
void CExtCaptureDlg::InitCtrls()
{
	...
	m_ckExtPushVideo.MoveWindow(140,150, 200, 24);
...
//		nBufferSize = 3 * videoInfo.bmiHeader.biHeight*videoInfo.bmiHeader.biWidth;
		...
		CAgoraObject::GetAgoraObject()->setExtendVideoCaptureMode(CAgoraObject::VIDEO_EXCAPTURE_OBSERVER);
...
//		m_agXAudioPlayoutDevice.SetAudioSampleRate(waveFormat.nSamplesPerSec);
	...
	if (m_ckExtPushVideo.GetCheck()) 
		CAgoraObject::GetAgoraObject()->setExtendVideoCaptureMode(CAgoraObject::VIDEO_EXCAPTURE_PUSH);
}
...
void CExtCaptureDlg::OnBnClickedCheckPushvideo()
{
	// TODO: Add your control notification handler code here
	bool bSelected = m_ckExtPushVideo.GetCheck();
	if (bSelected)
		m_ckExtVideoCapture.SetCheck(!bSelected);
}
