...
STDMETHODIMP CAGVideoCapture::BufferCB(double dblSampleTime, BYTE *pBuffer, long lBufferSize)
{
	...
	UINT nCapMode = lpAgoraObject->GetExtendVideoCaptureMode();

	switch (nCapMode)
	{
	case CAgoraObject::VIDEO_EXCAPTURE_OFF:
		break;
	case CAgoraObject::VIDEO_EXCAPTURE_OBSERVER:
		lpPackageQueue->PushVideoPackage(m_lpYUVBuffer, nYUVSize);
		break;
	case CAgoraObject::VIDEO_EXCAPTURE_PUSH: {
		lpAgoraObject->PushVideoFrame(m_lpYUVBuffer, bmiHeader.biWidth, bmiHeader.biHeight);
		int nYuvBufferLen = bmiHeader.biWidth * bmiHeader.biHeight * 3 / 2;
	}
											 break;
	default:
		break;
