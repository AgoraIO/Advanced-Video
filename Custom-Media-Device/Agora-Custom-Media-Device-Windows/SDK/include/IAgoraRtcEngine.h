...
{
    /* Default when you start a new screen capture. No hint has been provided, Agora Video Engine will make its best-informed guess on how contained video content should be treated. This can for example be inferred by doing content analysis.
     */
    CONTENT_HINT_NONE,
    /* The track should be treated as if it contains video where motion is important. This is normally webcam video, movies or video games. Quantization artifacts and downscaling are acceptable in order to preserve motion as well as possible while still retaining target bitrates. During low bitrates when compromises have to be made, more effort is spent on preserving frame rate than edge quality and details.
     */
    CONTENT_HINT_MOTION,
    /* The track should be treated as if video details are extra important. This is generally applicable to presentations or web pages with text content, painting or line art. This setting would normally optimize for detail in the resulting individual frames rather than smooth playback. Artifacts from quantization or downscaling that make small text or line art unintelligible should be avoided.
     */
    CONTENT_HINT_DETAILS
};

/**  Rectangle represent dimension and location of screen or view. This can be replaced by platform embedded structure. */
struct Rectangle
{
    // X coordinate
    int x;
    // Y coordinate
    int y;
    // Width of rectangle, as measured from the rectangle origin
    int width;
    // Height of rectangle, as measured from the rectangle origin
    int height;
...
/** Screen capture parameters. These parameters are used to control the
* video send stream of screen capture separately. while,
* setVideoEncoderConfiguration() is used for camera capture session.
*/
struct ScreenCaptureParameters
{
    /** The video frame dimension used to specify the video quality and
     * measured by the total number of pixels along a frame's width and
     * height:
     */
    VideoDimensions dimensions;
    /** 'ideal' video frame rate of screen capture.
     */
    int frameRate;
    /**'ideal' maximum bitrate, you might also choose STANDARD_BITRATE if you
     * don't know how to calculate maximum bitrate.
     */
    int bitrate;
...
public:
    ...
    */
    ...
    /** This method is used to share a whole screen or part of the screen by identifying a displayId.

     @param  displayId Sets the unique identifier of the display.
     @param  regionRect Sets the required region inside the display.
     @param  captureParams Sets the parameters of the capture
...
#if defined(_WIN32)
    /** This method is used to share whole screen or part of the screen by identifying a rectangle.

     @param  screenRect Sets the screen coordinate of the whole virtual screen.
     @param  regionRect Sets the required region inside the display.
     @param  captureParams Sets the parameters of the capture
    ...
    /** This method is used to share a window or a part of window by identifying a windowId.

     @param  windowId Sets the unique identifier of the window.
     @param  regionRect Sets the required region inside the window.
     @param  captureParams Sets the parameters of the capture
    ...
    /** This method is used to set content hint for current screen capture.  You must call this method after calling \ref startScreenCapture(), or the API will return error instead. If this method is not called, CONTENT_HINT_NONE will be used as default value.

     @param  contentHint Sets the category of the video content.
    ...
    /** This method pdates the screen capture parameters.
    ...
    /** This method updates the screen/window capture region.

     @param  regionRect Sets the required region inside the screen or window.
...
public:
    ...
    /** Specifies the size of the SDK output log file in KB.

    The log file records all SDK operations during runtime.

    @param fileSizeInKBytes Maximum file size in KB. The default value is 512.
    @return
