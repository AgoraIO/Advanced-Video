...
{
	TCHAR	szWavPath[MAX_DEVICE_ID_LENGTH];

	if (m_ptrDeviceManager == NULL)
		return;

	::GetModuleFileName(NULL, szWavPath, MAX_DEVICE_ID_LENGTH);
...
#ifdef UNICODE
		CHAR szWavPathA[MAX_DEVICE_ID_LENGTH];

		::WideCharToMultiByte(CP_ACP, 0, szWavPath, -1,szWavPathA , MAX_DEVICE_ID_LENGTH, NULL, NULL);
