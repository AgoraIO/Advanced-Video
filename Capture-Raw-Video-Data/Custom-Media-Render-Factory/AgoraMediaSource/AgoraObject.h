...
#define AVC_VER _T("V2.9.1, Build234, 09/11/2019, SDK2.9.1")
...
public:
	...
	BOOL EnableExtendAudioCapture(BOOL bEnable, int nSample, int nChannel);
	BOOL EnableExtendVideoCapture(BOOL bEnable,IExternalVideoRenderFactory* factory);

	UINT64 WINAPI GetHighAccuracyTickCount();
	BOOL PushVideoFrame(LPVOID pBufferFrame,int nBufferLen,int nWidth,int nHeight);
	BOOL PushAudioFrame(LPVOID pBufferFrame, int nBufferLen, const WAVEFORMATEX &wv);
	...
	void SetVideoSolution(int nWidth,int nHeight);
	void GetVideoSolution(int &nWidth,int &nHeight);

	void setVideoExternal(BOOL bEnableFactory) { m_bEnableFactory = bEnableFactory;}
	BOOL getVideoExternal() { return m_bEnableFactory; }
