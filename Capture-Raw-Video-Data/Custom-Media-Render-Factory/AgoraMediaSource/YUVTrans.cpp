...
BOOL CYUVTrans::CONVERT_ENDIAN_RGB24(LPBYTE const lpRGB24Buffer, int nWidth, int nHeight)
{
	for (int i = 0; i < nWidth; i++) {
		for (int j = 0; j < nHeight; j++) {
			LPBYTE pBuffer = lpRGB24Buffer + (i * nHeight + j) * 3;
			std::swap(pBuffer[0], pBuffer[2]);
		}
	}
...
BOOL CYUVTrans::CONVERT_BIG_RGB24(LPBYTE const lpRGB24Buffer,int nWidth, int nHeight)
{
	for (int i = 0; i < nWidth; i++) {
		for (int j = 0; j < nHeight; j++) {
			LPBYTE pBuffer = lpRGB24Buffer + (i * nWidth + j) * 3;
			std::swap(pBuffer[2], pBuffer[0]);
		}
	}
...
void CYUVTrans::VerMirror_RGB24(BYTE* pSrc, BYTE* pDest, int nWidth, int nHeight)
{
	int nStride = nWidth * 3;

	BYTE* pBufSrc = pSrc + nStride*(nHeight - 1);
	BYTE* pBufDst = pDest;
	for (int i = 0; i < nHeight; i++)
	{
		memcpy(pBufDst, pBufSrc, nStride);
...
void CYUVTrans::GetRGB24FromPic(const char* pPicPath, LPBYTE lpRGB24Buffer, int &nRGBBufSize, int &nWidth, int &nHeight)
{
	if (pPicPath) {
		...
		Bitmap* bmp = new Bitmap(CString(pPicPath));
		...
		for (int y = 0; y < height; y++)
			for (int x = 0; x < width; x++)
			{
				...
				lpRGB24Buffer[y * nWidth + x] = (BYTE)color.GetR();
				lpRGB24Buffer[y * nWidth + x + 1] = (BYTE)color.GetG();
				lpRGB24Buffer[y * nWidth + x + 2] = (BYTE)color.GetB();
			}

		nRGBBufSize = width * height * 3;
		nWidth = width;
		nHeight = height;
