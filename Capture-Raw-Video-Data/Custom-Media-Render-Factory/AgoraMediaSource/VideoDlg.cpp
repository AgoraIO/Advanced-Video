...
BEGIN_MESSAGE_MAP(CVideoDlg, CDialogEx)
	...
	ON_MESSAGE(WM_MSGID(EID_RENDER_FACTORY), &CVideoDlg::OnRenderFactory)
...
LRESULT CVideoDlg::OnEIDJoinChannelSuccess(WPARAM wParam, LPARAM lParam)
{
	...
	CAgoraObject::GetAgoraObject()->GetVideoSolution(nWidth, nHeight);
	m_wndLocal.SetVideoResolution(nWidth, nHeight);
...
LRESULT CVideoDlg::OnRenderFactory(WPARAM wParam, LPARAM lParam)
{
	LPAGE_RENDER_FACTORY lpData = reinterpret_cast<LPAGE_RENDER_FACTORY>(wParam);
	UNREFERENCED_PARAMETER(lParam);
	if (lpData) {
		if (lpData->pViewId == m_wndLocal.m_hWnd) {
			m_wndLocal.EnableRender(TRUE);
		}
		else {
			for (int nIndex = 0; nIndex < 4; nIndex++) {
				if (lpData->pViewId == m_wndVideo[nIndex].m_hWnd)
					m_wndVideo[nIndex].EnableRender(TRUE);;
			}
		}
