	...
	: CDialogEx(CExtCaptureDlg::IDD, pParent)
{
	...
	m_hExitPushEvent = ::CreateEvent(NULL, NULL, FALSE, NULL);
}
...
void CExtCaptureDlg::InitCtrls()
{
	...
	m_ckExtAudioCapture.MoveWindow(140, 312, 300, 24);
...
void CExtCaptureDlg::OnBnClickedBtnapplyExtcap()
{
	...
	if (m_ckExtVideoCapture.GetCheck()) {
		...
		CAgoraObject::GetAgoraObject()->SetVideoSolution(videoInfo.bmiHeader.biWidth, videoInfo.bmiHeader.biHeight);
	}
	CAgoraObject::GetAgoraObject()->setVideoExternal(m_ckExtVideoCapture.GetCheck());
	...
	if (m_ckExtAudioCapture.GetCheck()) {
		...
		m_nSample = waveFormat.nSamplesPerSec;
		...
		m_nSample = waveFormat.nSamplesPerSec;
		m_nChannel = waveFormat.nChannels;
		lpAgoraObject->SetAudioProfileEx(waveFormat.nSamplesPerSec, waveFormat.nChannels, waveFormat.nSamplesPerSec / 10);
...
{
	if (bStart) {
		return CAgoraObject::GetAgoraObject()->EnableExtendVideoCapture(TRUE,&m_renderFactory);
	}
	else {
		return CAgoraObject::GetAgoraObject()->EnableExtendVideoCapture(FALSE,nullptr);
	}
}
...
BOOL CExtCaptureDlg::AudioCaptureControl(BOOL bStart)
{
	...
	if (bStart) {
		CAgoraObject::GetAgoraObject()->EnableExtendAudioCapture(TRUE,m_nSample,m_nChannel);
		
		m_playThreadParam.hExitEvent = m_hExitPlayEvent;
		...
		m_pushThreadParam.hExitEvent = m_hExitPushEvent;
		AfxBeginThread(&CExtCaptureDlg::PushAudioThread, &m_pushThreadParam);

		return m_agAudioCaptureDevice.CaptureControl(DEVICE_START);
	}
	else {
		CAgoraObject::GetAgoraObject()->EnableExtendAudioCapture(FALSE,0,0);
		::SetEvent(m_hExitPlayEvent);
		::SetEvent(m_hExitPushEvent);
...
UINT CExtCaptureDlg::PlayoutThread(LPVOID lParam)
{
	...
	lpXAudioSoruceVoice->Start();

	WAVEFORMATEX wv;
	CAudioCapturePackageQueue::GetInstance()->GetAudioFormat(&wv);
	...
	do {
		...
		xAudioBuffer.pAudioData = reinterpret_cast<LPBYTE>(lpAudioData);
...
UINT CExtCaptureDlg::PushAudioThread(LPVOID lParam)
{
	LPPUSHAUDIO_THREAD_PARAM lpPushAudioThreadParam = reinterpret_cast<LPPUSHAUDIO_THREAD_PARAM>(lParam);
	CAudioPlayPackageQueue* pPlayPackageQueue = CAudioPlayPackageQueue::GetInstance();
	
	SIZE_T nAudioBufferSize = 8192;
	LPBYTE pBuffer = new BYTE[nAudioBufferSize];
	ZeroMemory(pBuffer, nAudioBufferSize);

	WAVEFORMATEX wv;
	pPlayPackageQueue->GetAudioFormat(&wv);

	do {
		if (WaitForSingleObject(lpPushAudioThreadParam->hExitEvent,2) == WAIT_OBJECT_0)
			break;

		pPlayPackageQueue->PopAudioPackage(pBuffer, &nAudioBufferSize);

		CAgoraObject::GetAgoraObject()->PushAudioFrame(pBuffer, nAudioBufferSize, wv);
	} while (TRUE);
