...
BOOL CAgoraObject::EnableExtendAudioCapture(BOOL bEnable,int nSample,int nChannel)
{
	...
	if (bEnable)
		m_lpAgoraEngine->setExternalAudioSource(true, nSample, nChannel);
	else
		m_lpAgoraEngine->setExternalAudioSource(false, 0, 0);

	return nRet == 0 ? TRUE : FALSE;
}

BOOL CAgoraObject::EnableExtendVideoCapture(BOOL bEnable, IExternalVideoRenderFactory* factory)
{
	agora::util::AutoPtr<agora::media::IMediaEngine> mediaEngine;
	mediaEngine.queryInterface(m_lpAgoraEngine, agora::AGORA_IID_MEDIA_ENGINE);
	...
	if (bEnable ) {
		if (factory)
			mediaEngine->registerVideoRenderFactory(factory);
	}
	else {
...
BOOL CAgoraObject::PushVideoFrame(LPVOID pBufferFrame, int nBufferLen, int nWidth, int nHeight)
{
	agora::util::AutoPtr<agora::media::IMediaEngine> mediaEngine;
	mediaEngine.queryInterface(m_lpAgoraEngine, agora::AGORA_IID_MEDIA_ENGINE);
	...
	LPBYTE lpYUV = reinterpret_cast<LPBYTE>(const_cast<LPVOID>(pBufferFrame));

	videoFrame.type = ExternalVideoFrame::VIDEO_BUFFER_TYPE::VIDEO_BUFFER_RAW_DATA;
	videoFrame.format = ExternalVideoFrame::VIDEO_PIXEL_FORMAT::VIDEO_PIXEL_I420;
	videoFrame.buffer = lpYUV;
	videoFrame.stride = nWidth;
	...
	videoFrame.timestamp = static_cast<long long>(GetHighAccuracyTickCount());

	int nRet = mediaEngine->pushVideoFrame(&videoFrame);
	return nRet == 0 ? TRUE : FALSE;
}

UINT64 WINAPI CAgoraObject::GetHighAccuracyTickCount()
{
	LARGE_INTEGER	liPC;
	LARGE_INTEGER	liPF;
...
BOOL CAgoraObject::PushAudioFrame(LPVOID pBufferFrame, int nBufferLen, const WAVEFORMATEX &wv)
{
	agora::util::AutoPtr<agora::media::IMediaEngine> mediaEngine;
	mediaEngine.queryInterface(m_lpAgoraEngine, agora::AGORA_IID_MEDIA_ENGINE);

	IAudioFrameObserver::AudioFrame af;
	af.samples = wv.nSamplesPerSec / 100;
	af.type = IAudioFrameObserver::FRAME_TYPE_PCM16;
	af.bytesPerSample = wv.wBitsPerSample/8;
	af.renderTimeMs = 10;
	af.samplesPerSec = wv.nSamplesPerSec;
	af.buffer = pBufferFrame;

	int nRet = mediaEngine->pushAudioFrame(MEDIA_SOURCE_TYPE::AUDIO_RECORDING_SOURCE, &af, true);
...
void CAgoraObject::SetVideoSolution(int nWidth, int nHeight)
{
	m_nWidth = nWidth;
	m_nHeight = nHeight;
}

void CAgoraObject::GetVideoSolution(int &nWidth, int &nHeight)
{
	nWidth = m_nWidth;
	nHeight = m_nHeight;
}