...
void CEnterChannelDlg::DoDataExchange(CDataExchange* pDX)
{
	...
	DDX_Control(pDX, IDC_CHECK_RENDER_FACTORY, m_btnCheckFactory);
}
...
//	MoveWindow(0, 0, 320, 450, 1);
	...
	m_btnCheckFactory.MoveWindow(80, 250, 320, 36, TRUE);
	m_btnExtCapture.MoveWindow(80, 300, 100, 36, TRUE);
    m_btnJoin.MoveWindow(80, 300, 320, 36, TRUE);
	...
	m_btnExtCapture.ShowWindow(SW_HIDE);
...
//	CString str = CAgoraObject::GetAgoraObject()->GetCallID();
	...
	m_dlgExtCapture.VideoCaptureControl(m_btnCheckFactory.GetCheck());
	m_dlgExtCapture.AudioCaptureControl(FALSE);
	CAgoraObject::GetAgoraObject()->setVideoExternal(m_btnCheckFactory.GetCheck());
