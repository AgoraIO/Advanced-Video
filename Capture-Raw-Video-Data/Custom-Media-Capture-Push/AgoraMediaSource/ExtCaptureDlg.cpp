...
UINT CExtCaptureDlg::PlayoutThread(LPVOID lParam)
{
	...
	WAVEFORMATEX wv;
	CAudioPlayPackageQueue::GetInstance()->GetAudioFormat(&wv);
...
UINT CExtCaptureDlg::PushAudioThread(LPVOID lParam)
{
	...
	WAVEFORMATEX wv;
	lpBufferQueue->GetAudioFormat(&wv);
	
	do {
		if (WaitForSingleObject(lpPushAudioThreadParam->hExitEvent,10) == WAIT_OBJECT_0)
			break;

		if (lpBufferQueue->PopAudioPackage(pBuffer, &nAudioBufferSize))
