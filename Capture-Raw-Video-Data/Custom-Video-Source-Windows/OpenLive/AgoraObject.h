...
//agora pushVideo
typedef struct _tagExternalVideoInfo {
...
public:
	...
	BOOL EnableVideoFrameObserver(bool bEnable); 
	BOOL EnableAudioFrameObserver(bool bEnable);

	// 外部采集
	//agora pushVideo
	BOOL SetExternalVideoSource(bool bEnable, bool bUserTexture=false);
	void StartPushExternalVideoFrame(CString yuvPath, int fps, agora::media::ExternalVideoFrame frame);
	void PushVideoFrame();
