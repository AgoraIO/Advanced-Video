...
BOOL CAgoraObject::EnableVideoFrameObserver(bool bEnable)
{
	...
	int ret = mediaEngine->registerVideoFrameObserver(nullptr);
	mediaEngine->release();

	return ret == 0 ? TRUE : FALSE;
}

BOOL CAgoraObject::EnableAudioFrameObserver(bool bEnable)
{
	agora::util::AutoPtr< agora::media::IMediaEngine> mediaEngine;
	mediaEngine.queryInterface(m_lpAgoraEngine, agora::AGORA_IID_MEDIA_ENGINE);
...
//agora pushVideo
BOOL CAgoraObject::SetExternalVideoSource(bool bEnable, bool bUserTexture)
{
	int ret = 0;
	agora::media::IMediaEngine* pMediaEngine = NULL;
	m_lpAgoraEngine->queryInterface(agora::AGORA_IID_MEDIA_ENGINE, (void**)&pMediaEngine);

	if (pMediaEngine) {
		ret = pMediaEngine->setExternalVideoSource(bEnable, false);
	}
...
void CAgoraObject::StartPushExternalVideoFrame(CString yuvPath, int fps, agora::media::ExternalVideoFrame frame)
{
	externalVideoInfo.frameRate = fps;
	externalVideoInfo.frame.type = frame.type;
	externalVideoInfo.frame.format = frame.format;
	externalVideoInfo.frame.stride = frame.stride;
	externalVideoInfo.frame.height = frame.height;
	externalVideoInfo.frame.cropLeft = frame.cropLeft;
	externalVideoInfo.frame.cropTop = frame.cropTop;
	externalVideoInfo.frame.cropRight = frame.cropRight;
	externalVideoInfo.frame.cropBottom = frame.cropBottom; 
	externalVideoInfo.frame.rotation = frame.rotation;
	
	_tfopen_s(&fpPushExternalVideo, yuvPath, _T("rb"));

	if (externalVideoInfo.frame.format == agora::media::ExternalVideoFrame::VIDEO_PIXEL_I420
...
void CAgoraObject::stopPushExternalVideoFrame()
{
	if (externalVideoInfo.frame.buffer) {
		delete[] externalVideoInfo.frame.buffer;
		externalVideoInfo.frame.buffer = NULL;
	}

	if (fpPushExternalVideo) {
		fclose(fpPushExternalVideo);
		fpPushExternalVideo = NULL;
	}

	externalVideoInfo.filePath = "";
}

void CAgoraObject::PushVideoFrame()
{
	...
	if (pMediaEngine) {
		...
		LARGE_INTEGER curTime, freq, timesatamp;
		QueryPerformanceFrequency(&freq);
		QueryPerformanceCounter(&curTime);
		curTime.QuadPart *= 1000;//ms
		curTime.QuadPart /= freq.QuadPart;
		externalVideoInfo.frame.timestamp = curTime.QuadPart;
		pMediaEngine->pushVideoFrame(&externalVideoInfo.frame);
		if (feof(fpPushExternalVideo)) {
			fseek(fpPushExternalVideo, 0, SEEK_SET);
		}
