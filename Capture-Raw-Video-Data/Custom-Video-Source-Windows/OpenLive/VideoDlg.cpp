...
BEGIN_MESSAGE_MAP(CVideoDlg, CDialogEx)
	...
	//ON_BN_CLICKED(IDC_BTNMORE_VIDEO + 10086, &CVideoDlg::OnBnClickedBtPushVideoFrame)
...
void CVideoDlg::ShowButtonsNormal()
{
	...
	//m_btnPush.ShowWindow(nShowMode); 
...
void CVideoDlg::AdjustButtonsNormal(int cx, int cy)
{
	...
	//if (m_btnPush.GetSafeHwnd() != NULL)
		//m_btnPush.MoveWindow(0, cy - 52, 90, 24, TRUE);
...
void CVideoDlg::InitCtrls()
{
	...
	//m_btnPush.Create(NULL, WS_VISIBLE | WS_CHILD, CRect(0, 0, 1, 1), this, IDC_BTNMORE_VIDEO + 10086);
	...
	//m_btnPush.SetBackColor(RGB(0x00, 0xA0, 0xE9), RGB(0x05, 0x78, 0xAA), RGB(0x05, 0x78, 0xAA), RGB(0xE6, 0xE6, 0xE6));
//	//m_btnPush.SetFont(&m_ftBtn);
	//m_btnPush.SetTextColor(RGB(0xFF, 0xFF, 0xFF), RGB(0xFF, 0xFF, 0xFF), RGB(0xFF, 0xFF, 0xFF), RGB(0xCC, 0xCC, 0xCC));
	
	//m_btnPush.SetWindowText(_T("pushVideo"));
...
void CVideoDlg::OnBnClickedBtPushVideoFrame()
{
	CString strText;
	//m_btnPush.GetWindowText(strText);

	if (strText.Compare(_T("pushVideo")) == 0){
		
		

		//m_btnPush.SetWindowText(_T("stopPush"));
	}
	else{
...
void CVideoDlg::pushVideoFrame()
{
	...
	CAgoraObject::GetAgoraObject()->StartPushExternalVideoFrame(strPath, fps, frame);

	SetTimer(10086, 1000 / fps, NULL);
}

void CVideoDlg::StopPushVideoFrame()
{
	KillTimer(10086);
	CAgoraObject::GetAgoraObject()->stopPushExternalVideoFrame();
}
void CVideoDlg::OnTimer(UINT_PTR nIDEvent)
{
	if (10086 == nIDEvent){
		CAgoraObject::GetAgoraObject()->PushVideoFrame();
	}
}
//end
