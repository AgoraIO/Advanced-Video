...
struct RTPVideoHeaderVP8
{
    ...
    uint8_t        resolutionIndex;// VP8 codec resolution identifier
};
...
struct RTPVideoHeader
{
  ...
  bool              isLastpacket;    // last packet in frame
  uint8_t           simulcastIdx;    // Index if the simulcast encoder creating
                                      // this frame, 0 if not using simulcast.
  RtpVideoCodecTypes      codec;
  RTPVideoTypeHeader      codecHeader;
  uint8_t           agoraVideoVersion;
  int32_t           seqInFrame;
  int32_t           fecPkgs;
  int32_t           totalPkgs;
  int32_t           fecMethod;
...
struct VideoContentMetrics {
  ...
  VideoContentMetrics(const VideoContentMetrics &src) {
    motion_magnitude = src.motion_magnitude;
  spatial_pred_err = src.spatial_pred_err;
  spatial_pred_err_h = src.spatial_pred_err_h;
  spatial_pred_err_v = src.spatial_pred_err_v;
  }
...
enum RawAudioUseMode {
  kModeRead,
  kModeWrite,
  kModeBoth,
};

typedef struct RawAudioFormatStruct {
public:
  RawAudioFormatStruct() {
    sampleRate = -1;
    channelCnt = -1;
    pullInterval = -1;
    useMode = kModeBoth;
...
public:
    ...
    AudioFrame& operator-=(const AudioFrame& rhs);
    AudioFrame& operator*=(const float rhs);
  ...
  //Strong, add two more flags
  //float frame_snr_;
  float frame_noise_power_;
...
inline AudioFrame::AudioFrame()
    ...
    energy_(0xffffffff),
  //frame_snr_(0.0f),
  frame_noise_power_(0.0f)
{
}

inline AudioFrame::~AudioFrame() {}

inline void AudioFrame::UpdateFrame(
    int id,
    uint32_t timestamp,
    const int16_t* data,
    int samples_per_channel,
...
inline void AudioFrame::CopyFrom(const AudioFrame& src)
{
    ...
    energy_           = src.energy_;
  //frame_snr_      = src.frame_snr_;
  frame_noise_power_ = src.frame_noise_power_;
...
inline void AudioFrame::Mono2Stereo()
{
	/**
	 * We have a memory leak here because the memory allocated
	 * for tmp does not been deallocated at all. In fact, we
	 * can just use the temporay local array for the temporary
	 * memory.
	 * -- by Lionfore Hao(@haolianfu@agora.io) Mar 14th, 2018
	 **/
	//int16_t* tmp = new int16_t[samples_per_channel_];
	int16_t tmp [kMaxDataSizeSamples]; /* Fixed the memory leak by Lionfore. Mar 14th, 2018 */
	assert (samples_per_channel_ <= kMaxDataSizeSamples / 2); /* Do this assertion here. Lionfore, Mar 14th, 2018 */
	memcpy (tmp, data_, sizeof (int16_t) * samples_per_channel_);
...
inline void AudioFrame::MuteAll()
{
  memset(data_, 0, kMaxDataSizeSamples * sizeof(int16_t));
}
...
inline AudioFrame& AudioFrame::operator*=(const float rhs)
{
    assert((num_channels_ > 0) && (num_channels_ < 3));
    if((num_channels_ > 2) ||
        (num_channels_ < 1))
