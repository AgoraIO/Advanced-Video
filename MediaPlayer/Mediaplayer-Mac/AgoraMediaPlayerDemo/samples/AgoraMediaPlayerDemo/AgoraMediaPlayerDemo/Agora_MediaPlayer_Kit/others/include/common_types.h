...
#ifdef WEBRTC_EXPORT
#define WEBRTC_DLLEXPORT HELPER_DLL_EXPORT
#elif WEBRTC_DLL
#define WEBRTC_DLLIMPORT HELPER_DLL_IMPORT
#else
...
namespace AgoraExternalResampler {
  ...
  enum TraceModule
  {
    ...
    kTraceVideoEngine = 0x0013,
    kTraceVideoRenderer = 0x0014,
    kTraceVideoCapture = 0x0015,
    kTraceVideoPreocessing = 0x0016,
    kTraceRemoteBitrateEstimator = 0x0017,
    kTraceMediaSdk = 0x0100,
    kTraceChatEngine = 0x0101
  };
  ...
  enum TraceLevel
  {
    ...
    kTraceDefault = 0x003f,
    kTraceQuality = 0x0040,  // for fec info | retransmission info etc.
  ...
  enum FrameType
  {
    ...
    kVideoFrameAltRef = 6,
    kVideoFrameB = 7,
    kAudioFrameNotSend = 8,    // for agora dtx framework
    kAudioFrameBroadcast = 9,    // for agora dtx framework
  };
  
  struct PacketInfoInternal
  {
  ...
  {
  public:
    virtual int SendPacket(int channel, PacketInfoInternal& pkg_info) = 0;
  ...
  {
    kDtxOff = 0,
    kDtxOnZeroPayload = 1,        //dtx enabled and keep sending packages
    kDtxOnPackNotSend = 2,        //dtx enabled and not sending package when silience
  };
  ...
  struct VideoCodec
  {
    ...
    unsigned short      height_low;
    unsigned short      framerate_low;
    unsigned int        bitrate_low;

#if defined(WEBRTC_ANDROID)
    /*
     the following two variable used for small stream Android hardware encoder,
     some android phone doesn't support very low resolution, for example:192x108
     we must padding low resolution image to large resolution, and then crop the
     padding part on decoder side.
     these two variable mark the origin low resolution.
    */
    unsigned short      hwenc_origin_width;
    unsigned short      hwenc_origin_height;
#endif
  ...
  public:
    ...
    //       of AgoraExternalResampler::kViEMaxMtu if you are encrypting in the video engine, or
    //       AgoraExternalResampler::kVoiceEngineMaxIpPacketSizeBytes for the voice engine.
