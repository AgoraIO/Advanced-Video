...
#ifndef WEBRTC_LOGGING
#define WEBRTC_LOGGING
#endif

#if !defined(WEBRTC_LOGGING)
#define WEBRTC_TRACE (true) ? (void)0 : AgoraExternalResampler::Trace::Add
#define WEBRTC_TRACE_STREAM (true) ? (void)0 : AgoraExternalResampler::Trace::Add
#else
#if defined(DEBUG)
#define WEBRTC_TRACE AgoraExternalResampler::Trace::Add
#else
#define WEBRTC_TRACE(level, ...) \
    do {if(level <= AgoraExternalResampler::kTraceDebug) \
         AgoraExternalResampler::Trace::Add(level, ##__VA_ARGS__);} while(0)
#endif

#define WEBRTC_TRACE_STREAM(...) if (false) AgoraExternalResampler::Trace::Add(kTraceStream, ##__VA_ARGS__)
#define WEBRTC_TRACE_DBGINFO(...) if (false) AgoraExternalResampler::Trace::Add(kTraceInfo, ##__VA_ARGS__)
//#define WEBRTC_TRACE_STREAM(...) AgoraExternalResampler::Trace::Add(kTraceStream, ##__VA_ARGS__)
#endif
...
class Trace {
 public:
  ...
  static int32_t SetTraceFile(const char* file_name,
                              const int max_file_counter = 1,
                const int max_file_size = 0);
