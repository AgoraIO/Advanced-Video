...
void CAgoraObject::SetScreenCaptureEnable(BOOL bEnable /*= TRUE*/)
{
	m_bScreenCapture = bEnable;
}
...
BOOL CAgoraObject::MuteRemoteAudio(UINT uRemoteID, BOOL bMute /*= TRUE*/)
{
	ASSERT(m_lpAgoraEngine != NULL);

	RtcEngineParameters rep(*m_lpAgoraEngine);

	int ret = rep.muteRemoteAudioStream(uRemoteID, (bool)bMute);

	return ret == 0 ? TRUE : FALSE;
}

BOOL CAgoraObject::MuteRemoteVideo(UINT uRemoteID,BOOL bMute /*= TRUE*/)
{
	ASSERT(m_lpAgoraEngine != NULL);

	RtcEngineParameters rep(*m_lpAgoraEngine);

	int ret = rep.muteRemoteVideoStream(uRemoteID, (bool)bMute);
