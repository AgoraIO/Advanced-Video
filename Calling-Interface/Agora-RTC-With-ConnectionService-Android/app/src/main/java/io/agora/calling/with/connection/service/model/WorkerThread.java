...
import io.agora.calling.with.connection.service.R;
import io.agora.calling.with.connection.service.cs.AgoraConnectionService;
import io.agora.calling.with.connection.service.cs.CallSession;
...
public class WorkerThread extends Thread {
    ...
    public final void makeACall(final String peerUid, final String channel) {
        ...
        placeCall(String.valueOf(mEngineConfig.mUid & 0xFFFFFFFFL), channel, peerUid);
    ...
    private void registerPhoneAccount(Context context) {
        PhoneAccountHandle accountHandleIn = new PhoneAccountHandle(
                new ComponentName(context, AgoraConnectionService.class), ConstantApp.CALL_IN_CS);
        PhoneAccountHandle accountHandleOut = new PhoneAccountHandle(
                new ComponentName(context, AgoraConnectionService.class), ConstantApp.CALL_OUT_CS);


        PhoneAccount.Builder paBuilder;
        paBuilder = PhoneAccount.builder(accountHandleIn, ConstantApp.CALL_IN_CS)
                .setCapabilities(PhoneAccount.CAPABILITY_CONNECTION_MANAGER); // PhoneAccount.CAPABILITY_SELF_MANAGED If apply totally customized in-call UI
        PhoneAccount phoneIn = paBuilder.build();

        paBuilder = PhoneAccount.builder(accountHandleOut, ConstantApp.CALL_OUT_CS)
                .setCapabilities(PhoneAccount.CAPABILITY_CONNECTION_MANAGER); // PhoneAccount.CAPABILITY_SELF_MANAGED If apply totally customized in-call UI
        ...
        CallSession cSession = CallSession.getInstance();
        cSession.setPhoneAccountIn(phoneIn);
        cSession.setPhoneAccountOut(phoneOut);

        log.debug("registerPhoneAccount " + context + " " + cSession);
    }

    private void placeCall(String myUid, String channel, String subscriber) {
        Bundle extras = new Bundle();
        extras.putInt(TelecomManager.EXTRA_START_CALL_WITH_VIDEO_STATE, VideoProfile.STATE_BIDIRECTIONAL);
        Bundle extraBundle = new Bundle();
        extraBundle.putString(ConstantApp.ACTION_KEY_UID, myUid);
        extraBundle.putString(ConstantApp.ACTION_KEY_SUBSCRIBER, subscriber);
        extraBundle.putString(ConstantApp.ACTION_KEY_CHANNEL_NAME, channel);
        extraBundle.putInt(ConstantApp.ACTION_KEY_MakeOrReceive, ConstantApp.CALL_OUT);
        extras.putBundle(TelecomManager.EXTRA_OUTGOING_CALL_EXTRAS, extraBundle);

        log.debug("placeCall account: " + myUid + " subscriber: " + subscriber + " channel: " + channel);
        ...
        try {
            ...
            telecomManager.placeCall(
                    Uri.fromParts(AgoraConnectionService.SCHEME_AG, subscriber, null), extras);
        } catch (SecurityException e) {
            log.error("Unable to placeCall: " + e);
        }
    }

    public void addNewIncomingCall(String myUid, String channel, String subscriber) {
        Bundle extras = new Bundle();
        extras.putString(ConstantApp.ACTION_KEY_UID, myUid);
        extras.putString(ConstantApp.ACTION_KEY_SUBSCRIBER, subscriber);
        extras.putString(ConstantApp.ACTION_KEY_CHANNEL_NAME, channel);
        extras.putInt(ConstantApp.ACTION_KEY_MakeOrReceive, ConstantApp.CALL_IN);
        extras.putInt(TelecomManager.EXTRA_START_CALL_WITH_VIDEO_STATE, VideoProfile.STATE_BIDIRECTIONAL);

        log.debug("addIncomingCall account: " + myUid + " subscriber: " + subscriber + " channel: " + channel);
        ...
        try {
            ...
            log.error("Unable to addNewIncomingCall: " + e);
        }
    }
