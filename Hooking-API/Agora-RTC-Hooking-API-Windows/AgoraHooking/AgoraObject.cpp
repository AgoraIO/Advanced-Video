...
BOOL CAgoraObject::SetVideoPreRotation(BOOL bEnable)
{
	int nRet = 0;
	AParameter ap(m_lpAgoraEngine);

	if (bEnable)
		nRet = ap->setParameters("{\"che.video.keep_prerotation\":true");
	else 
		nRet = ap->setParameters("{\"che.video.keep_prerotation\":false");
