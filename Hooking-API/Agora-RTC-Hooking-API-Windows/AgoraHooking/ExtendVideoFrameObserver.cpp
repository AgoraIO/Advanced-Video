...
{
	static FILE* pFileYUVSrc = fopen("..\\Extendvideo.yuv", "rb+");
	if (NULL == pFileYUVSrc)
		return true;
	...
	int nRet = fread(m_lpImageBuffer, 1, nBufferSize, pFileYUVSrc);
	if (feof(pFileYUVSrc) || nRet < nBufferSize )
		fseek(pFileYUVSrc, 0, SEEK_SET);
