...
void CEnterChannelDlg::DoDataExchange(CDataExchange* pDX)
{
	...
	DDX_Control(pDX, IDC_CHECK_STRINGID, m_chkStringUid);
	DDX_Control(pDX, IDC_EDIT_STRINGUID, m_edtStringUid);
}
...
BEGIN_MESSAGE_MAP(CEnterChannelDlg, CDialogEx)
	...
	ON_BN_CLICKED(IDC_CHECK_STRINGID, &CEnterChannelDlg::OnBnClickedCheckStringid)
END_MESSAGE_MAP()
...
void CEnterChannelDlg::InitCtrls()
{
	...
	m_edtStringUid.MoveWindow(ClientRect.Width() / 2 - 150, 127, 300, 22, TRUE);
	m_edtStringUid.SetFont(&m_ftDesc);
	m_edtStringUid.SetCaretPos(CPoint(12, 148));
	m_edtStringUid.ShowCaret();
	m_edtStringUid.SetTip(_T("StringUID"));
	m_edtStringUid.SetFocus(); 
	m_ctrChannel.SetFocus();
	if (!m_chkStringUid.GetCheck()){
	//	m_edtStringUid.ShowWindow(SW_HIDE);
	}

	m_ctrRole.SetCurSel(0);
   
	m_chkStringUid.MoveWindow(ClientRect.Width() / 2 - 270, 264, 120, 36, TRUE);
	m_chkStringUid.ShowWindow(SW_HIDE);
...
void CEnterChannelDlg::OnBnClickedBtnjoinChannel()
{
	...
	GetParent()->SendMessage(WM_JOINCHANNEL, 0, m_chkStringUid.GetCheck());
}
...
CString CEnterChannelDlg::GetStringUid()
{
	CString stringUid;

	m_edtStringUid.GetWindowText(stringUid);
...
void CEnterChannelDlg::OnBnClickedCheckStringid()
{
	// TODO: Add your control notification handler code here

	if (m_chkStringUid.GetCheck()){
		m_edtStringUid.ShowWindow(SW_SHOW);
	}
	else{
		m_edtStringUid.ShowWindow(SW_HIDE);
	}
