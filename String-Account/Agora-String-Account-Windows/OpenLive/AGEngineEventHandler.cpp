...
{
	LPAGE_LOCAL_USER_REGISTER lpData = new AGE_LOCAL_USER_REGISTER;
	lpData->uid = uid;
	strcpy_s(lpData->userAccount, 256, userAccount);
	lpData->userAccount[strlen(userAccount)] = 0;
	if (m_hMainWnd != NULL)
		::PostMessage(m_hMainWnd, WM_MSGID(EID_LOCAL_USER_REGISTER), (WPARAM)lpData, 0);
	
}

void CAGEngineEventHandler::onUserInfoUpdated(uid_t uid, const UserInfo& info)
{
	LPAGE_USER_INFO_UPDATE lpData = new AGE_USER_INFO_UPDATE;
	lpData->uid = uid;
	strcpy_s(lpData->userAccount, 256, info.userAccount);
	lpData->userAccount[strlen(info.userAccount)] = 0;
	if (m_hMainWnd != NULL)
		::PostMessage(m_hMainWnd, WM_MSGID(EID_USER_INFO_UPDATE), (WPARAM)lpData, 0);
}