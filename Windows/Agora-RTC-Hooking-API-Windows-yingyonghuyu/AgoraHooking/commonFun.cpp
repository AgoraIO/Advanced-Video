...
{
	TCHAR path[MAX_DEVICE_ID_LENGTH] = { 0 };
	GetModuleFileName(nullptr, path, MAX_DEVICE_ID_LENGTH);
...
std::string getFileAbsolutePath(const std::string &file)
{
	...
	TCHAR path[MAX_DEVICE_ID_LENGTH] = { 0 };
	GetModuleFileName(hModule, path, MAX_DEVICE_ID_LENGTH);
	return CStringA(path).GetBuffer();
}
...
{
	char str[MAX_DEVICE_ID_LENGTH] = { 0 };
	_itoa_s(nNum, str, 10);
	return str;
}
...
std::string getMediaSdkLogPath()
{
	...
	exeName.append("AgoraHOOK_Media");
	exeName.append(".log");
