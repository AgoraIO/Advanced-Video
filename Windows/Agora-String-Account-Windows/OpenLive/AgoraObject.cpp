...
BOOL CAgoraObject::JoinChannelWithUserAccount(LPCTSTR channelId, LPCTSTR userAccount, LPCTSTR token)
{
	CHAR szChannelName[128] = { 0 };
	CHAR szUserAccount[256] = { 0 };
	CHAR szUserToken[256] = { 0 };

	::WideCharToMultiByte(CP_ACP, 0, channelId, -1, szChannelName, 128, NULL, NULL);
	::WideCharToMultiByte(CP_ACP, 0, userAccount, -1, szUserAccount, 256, NULL, NULL);
	::WideCharToMultiByte(CP_ACP, 0, token, -1, szUserToken, 256, NULL, NULL);

	int nRet = m_lpAgoraEngine->joinChannelWithUserAccount(NULL, szChannelName, szUserAccount);
	if (nRet == 0)
		m_strChannelName = channelId;
	
	return nRet == 0 ? TRUE : FALSE;
}

void CAgoraObject::SetUserAccountParameter()
{
	AParameter apm(m_lpAgoraEngine); 

	apm->setParameters("{\"rtc.user_account_server_list\":[\"58.211.82.170\"]}");
}

BOOL CAgoraObject::RegisterLocalUserAccount(LPCTSTR strAppId, LPCTSTR userAccount)
{
	CHAR szAppid[128] = { 0 };
	CHAR szUserAccount[256] = { 0 };
	
	::WideCharToMultiByte(CP_ACP, 0, strAppId, -1, szAppid, 128, NULL, NULL);
	::WideCharToMultiByte(CP_ACP, 0, userAccount, -1, szUserAccount, 256, NULL, NULL);
	

	int nRet = m_lpAgoraEngine->registerLocalUserAccount(szAppid, szUserAccount);
