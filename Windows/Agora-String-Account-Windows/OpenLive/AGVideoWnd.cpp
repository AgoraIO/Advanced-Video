...
void CAGInfoWnd::OnPaint()
{
	...
	if (m_bShowTip) {
		...
		rcClient.top += 4;
		if (!m_sUid.IsEmpty())
			strTip.Format(_T("suid:%s, %dx%d, %dfps, %dK"), m_sUid, m_nWidth, m_nHeight, m_nFps, m_nBitrate);
		else
...
void CAGVideoWnd::SetBigShowFlag(BOOL bBigShow)
{
	...
	if (m_wndInfo.GetSafeHwnd() != NULL) {
		...
		if (m_sUid.IsEmpty())
			...
			m_wndInfo.MoveWindow(x, y, 252, 24);
	}
};
...
void CAGVideoWnd::SetStringUid(CString sUid)
{
	m_sUid = sUid; 
	m_wndInfo.SetStringUid(sUid);
	SetBigShowFlag(m_bBigShow);
}