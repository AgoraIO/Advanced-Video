...
BEGIN_MESSAGE_MAP(CVideoDlg, CDialogEx)
	...
	ON_MESSAGE(WM_MSGID(EID_LOCAL_USER_REGISTER), &CVideoDlg::OnLocalUserRegistered)

	ON_MESSAGE(WM_MSGID(EID_USER_INFO_UPDATE), &CVideoDlg::OnUserInfoUpdated)
...
LRESULT CVideoDlg::OnEIDUserOffline(WPARAM wParam, LPARAM lParam)
{
	...
	while (pos != NULL){
		if (m_listWndInfo.GetAt(pos).nUID == lpData->uid) {
			...
			RebindVideoWnd();
			auto iter = m_mapUidToStringUid.find(lpData->uid);
			if (iter != m_mapUidToStringUid.end())
				m_mapUidToStringUid.erase(iter);
			break;
		}
...
LRESULT CVideoDlg::OnRemoteVideoStat(WPARAM wParam, LPARAM lParam)
{
	...
	while (posNext != NULL) {
		...
		if (rWndInfo.nUID == lpData->uid) {
			...
			if (m_mapUidToStringUid.size()
				&& m_mapUidToStringUid.find(lpData->uid) != m_mapUidToStringUid.end()){
				m_wndVideo[rWndInfo.nIndex].SetStringUid(m_mapUidToStringUid[lpData->uid]);
			}
...
void CVideoDlg::RebindVideoWnd()
{
	...
	for (int nIndex = 0; nIndex < 4; nIndex++) {
		if (pos != NULL) {
			...
			m_wndVideo[nIndex].SetStringUid(_T(""));
		}
...
LRESULT CVideoDlg::OnLocalUserRegistered(WPARAM wParam, LPARAM lParam)
{
	LPAGE_LOCAL_USER_REGISTER lpData = (LPAGE_LOCAL_USER_REGISTER)wParam;
...
LRESULT CVideoDlg::OnUserInfoUpdated(WPARAM wParam, LPARAM lParam)
{
	LPAGE_USER_INFO_UPDATE lpData = (LPAGE_USER_INFO_UPDATE)wParam;

	TCHAR szUserAccount[256] = { 0 };

	MultiByteToWideChar(CP_ACP, 0, lpData->userAccount, -1, szUserAccount, 256);

	m_mapUidToStringUid.insert(std::make_pair(lpData->uid, szUserAccount));
	delete lpData;
	lpData = NULL;
	return 0;
}
